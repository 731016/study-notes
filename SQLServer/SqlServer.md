...
1. 1. 统一码（Unicode）2.0标志中所定义的字符，包括拉丁字母a~z和A~Z，以及来自其他语言的字符
...
| `[unique][clustered|nonclustered]` | `指定创建索引的类型，参数依次为唯一索引、聚集索引、非聚集索引    当省略unique选项时，建立非聚集索引    省略[clustered|nonclustered]选项时，建立聚集索引    省略nonclustered选项时，建立唯一聚集索引` |
| ---------------------------------- | ------------------------------------------------------------ |
